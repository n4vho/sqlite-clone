...
typedef struct {
    StatementType type;
    Row row_to_insert; // only used by insert srtatement
} Statement;
...
void* get_page(Pager* pager, uint32_t page_num) {
    ...
    if (page_num > TABLE_MAX_PAGES) {
        printf("Tried to fetch page numer out of bounds. %d > %d\n", page_num, TABLE_MAX_PAGES);
        exit(EXIT_FAILURE);
    }
...
void db_close(Table* table) {
    ...
    // might be a partial page to write to the EOF 
    // wont need it anymore after i implement b-tree
...
void* row_slot(Table* table, uint32_t row_num) {
    ...
    if (page == NULL) {
        // allocating memory only when accessing page
